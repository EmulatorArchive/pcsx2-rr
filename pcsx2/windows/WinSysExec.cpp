...
__forceinline void RunFrame()
{
	...
	if(g_Movie.File) {
		if(g_Movie.Replay) {
			Console::SetTitle(fmt_string("Play - %d/%d", g_Movie.FrameNum, g_Movie.FrameMax));
			if(g_Movie.FrameNum >= g_Movie.FrameMax)
				MovieEnd();
		} else {
			Console::SetTitle(fmt_string("Record - %d", g_Movie.FrameNum));
			g_Movie.FrameMax = g_Movie.FrameNum;
		}
	} else
		Console::SetTitle(fmt_string("%d", g_Movie.FrameNum));
}
//-------//
...
void ExecuteCpu()
{
	...
	while( !g_ReturnToGui )
	{
		...
		if( !g_Movie.Paused )
			RunFrame();
		else if(g_FAdvNext && (GetTickCount() >= g_FAdvNext)) {
			g_FAdvNext += 50;
			RunFrame();
		} else {
			gsPostVsyncEnd(true);
...
static void __fastcall KeyEvent(keyEvent* ev)
{
	...
	if (ev->evt == KEYRELEASE) {
		switch (ev->key) {
			...
			case VK_SPACE: g_FAdvNext = 0; break; // TAS
		}
	...
	switch (ev->key) {
		...
		//--TAS--//
		case 'P':
			g_Movie.Paused = !g_Movie.Paused;
			break;

		case 'R':
			g_Movie.ReadOnly = !g_Movie.ReadOnly;
			Console::Notice("Read-only %s",params (g_Movie.ReadOnly?"on":"off"));
			break;

		case VK_SPACE:
			g_Movie.Paused = true;
			RunFrame();
			g_FAdvNext = GetTickCount()+200;
			break;
		//-------//
