...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		memset(&g_Movie, 0, sizeof(g_Movie)); // TAS
		SysInit();
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
			...
			case ID_MOVIE_RECORD:
				if(DialogBox(gApp.hInstance, MAKEINTRESOURCE(IDD_RECORD), hWnd, (DLGPROC)RecordDlgProc))
					RunExecute(NULL);
				break;

			case ID_MOVIE_PLAY:
				if(DialogBox(gApp.hInstance, MAKEINTRESOURCE(IDD_PLAY), hWnd, (DLGPROC)PlayDlgProc))
					RunExecute(NULL);
				break;
